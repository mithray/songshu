...
const encrypt = require('cryptsec').encrypt
...
async function set(plaintext_key, plaintext_val) {
    ...
    try {
        encrypted_value = await encrypt(plaintext_val, encryption_key)
        encKey = await getEncryptedPropertyKey(plaintext_key)
        encrypted_key = (await encKey)
            ? await encKey
            : await encrypt(plaintext_key, encryption_key)
        try {
